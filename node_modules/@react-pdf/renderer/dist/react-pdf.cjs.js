...
var Yoga = _interopDefault(require('@react-pdf/yoga'));
...
class FontSource {
  ...
  async load() {
    ...
    if (isDataUrl(this.src)) {
      ...
      this.data = await new Promise((resolve, reject) => fontkit.open(this.src, postscriptName, (err, data) => err ? reject(err) : resolve(data)));
    }
...
const fontSubstitution = () => ({
  ...
  for (const run of runs) {
    ...
    for (const char of string.slice(run.start, run.end)) {
      ...
      const shouldFallback = shouldFallbackToFont(codePoint, defaultFont); // If the default font does not have a glyph and the fallback font does, we use it
...
const getFragments = (instance, parentLink) => {
  if (!instance) return [{
    string: ''
  }];
  let fragments = [];
...
const defaultPresenceAhead = element => height => Math.min(element.height, height); // Calculates the presence ahead or an array of nodes, given the available height.
...
const wrap = async (elements, height, pageNumber) => {
  ...
  for (let i = 0; i < elements.length; i++) {
    ...
    let elementShouldBreak = element.break || !element.wrap && elementShouldSplit && elementFitsFullPageHeight; // If element is fixed, we add it both to the current page
...
const wrapPages = async (nodes, height, pageIndex) => {
  ...
  const hasOnlyFixedChilds = nextPage.children.length > 0 && nextPage.children.every(c => c.fixed);
  ...
  async wrapPages() {
    ...
    for (const page of this.children) {
      if (page.wrap) {
        ...
        const subpages = await wrapPage(page, wrapArea, pageCount);
        pageCount += subpages.length;
        subpages.forEach((sub, index) => pages.push({
          page: sub,
          number: index + 1,
...
const availableMethods = ['dash', 'clip', 'save', 'path', 'fill', 'font', 'text', 'rect', 'scale', 'moveTo', 'lineTo', 'stroke', 'rotate', 'circle', 'lineCap', 'opacity', 'ellipse', 'polygon', 'restore', 'lineJoin', 'fontSize', 'fillColor', 'lineWidth', 'translate', 'miterLimit', 'strokeColor', 'fillOpacity', 'roundedRect', 'strokeOpacity', 'fillAndStroke', 'bezierCurveTo', 'quadraticCurveTo'];
